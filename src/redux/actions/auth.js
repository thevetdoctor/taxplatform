...
export const registerUser =
  (data, setLoading, setErrors, history) => async (dispatch) => {
    try {
      setLoading(true);
...
export const loginUser =
  (data, setLoading, setErrors, rememberMe) => async (dispatch) => {
    try {
      setLoading(true);
...
export const resetPasswordRequest =
  (data, setLoading, setErrors, setSuccess) => async (dispatch) => {
    try {
      ...
      if (res) {
        dispatch(setApplicationSuccess(res.data?.message));
        setLoading(false);
      }
    } catch (err) {
...
export const createNewPassword =
  (data, setLoading, history) => async (dispatch) => {
    try {
      console.log("try");
      setLoading(true);
      ...
      if (res) {
        console.log(res.data);
        dispatch(setApplicationSuccess("Password Reset Sent to Email!"));
      }
    } catch (err) {
      console.log(err);
      if (err && err.response) {
